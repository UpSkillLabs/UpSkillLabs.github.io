...
function showCard() {
  ...
  flipped = false;
const frontBody = document.querySelector('.card-front .card-body');
if (frontBody) frontBody.scrollTop = 0;  const tc = TYPE_CONFIG[c.type] || { cls: 'type-def', label: c.type || '' };
...
function flipCard() {
  ...
  // reset scroll on whichever face just became visible
  const activeBody = card.querySelector(
    flipped ? '.card-back .card-body' : '.card-front .card-body'
  );
...
function initSwipe() {
  ...
  stage.addEventListener('touchmove', e => {
    ...
    if (Math.abs(dx) > Math.abs(dy) + 8) {
      ...
      card.style.transform  = `rotate(${rot}deg) translateX(${dx * 0.25}px)`;
